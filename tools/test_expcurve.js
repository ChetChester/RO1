...
/* ---------- 100~200 的延伸段 + 等級上限 + 打寶模式（#110）---------- */
{
  ...
  /* 曲線的**形狀**是這次改動的重點（照官方三轉的後置感），總量則是校準目標。
     這兩條寫死數字沒有意義——會壞的是「改了某一段係數之後形狀跑掉」。 */
  const tot = cum(100, 200);
  t.ok('190→200 就吃掉六成以上（官方 66.8%）',
    cum(190, 200) / tot > 0.6, (cum(190, 200) / tot * 100).toFixed(1) + '%');
  t.ok('150 之前只佔個位數百分比',
    cum(100, 150) / tot < 0.05, (cum(100, 150) / tot * 100).toFixed(1) + '%');

  /* 校準：打寶一般檔（經驗 ×5）三個月。每日預算＝2h 主動 + 22h 離線。
     實測主動 287 exp/秒（由「Lv80→99 三小時」反推），離線約其 24%。 */
  const ACTIVE = 287, OFFLINE = 287 * 1.15 / 4.75;
  const daily = m => (2 * 3600 * ACTIVE + 22 * 3600 * OFFLINE) * m;
  const months = m => tot / daily(m) / 30;
  t.ok('打寶一般檔約 3 個月', Math.abs(months(5) - 3) < 0.4, months(5).toFixed(2) + ' 個月');
  t.ok('瘋狂檔更快', months(10) < months(5));
  t.ok('不開打寶明顯更久（打寶是主要管道）', months(1) > months(5) * 4);
